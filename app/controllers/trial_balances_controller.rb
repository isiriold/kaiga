class TrialBalancesController < ApplicationController
  ...
  def show
    @trial_balance_array = []
    credit_sum_arr = user_default_branch.account_transaction_items.find(:all,
    ...
    debit_sum_arr = user_default_branch.account_transaction_items.find(:all,
                                                               :select => "account_id, SUM(amount) as amount_sum",
    ...
    accounts.each do |account|                        
      ...
      @trial_balance_array << {"account_name" => account.name, "amount" => difference, "type" => type}            
    end
  end
end
