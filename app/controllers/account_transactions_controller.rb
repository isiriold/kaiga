class AccountTransactionsController < ApplicationController
  ...
  def index
    @account_transactions = user_default_branch.account_transactions.all(:include => [:account_transaction_items])
  ...
  def show
    @account_transaction = user_default_branch.account_transactions.find(params[:id])
  ...
  def new
    @account_transaction = user_default_branch.account_transactions.build
    5.times do
  ...
  def create
    @account_transaction = user_default_branch.account_transactions.build(params[:account_transaction])
    @account_transaction.transaction_date = current_day.for_date
