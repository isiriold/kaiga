...
describe "/account_sub_groups/new.html.erb" do
  include AccountSubGroupsHelper
  
  before(:each) do
    assigns[:account_sub_group] = stub_model(AccountSubGroup,
      :new_record? => true,
      :name => "value for name",
  ...
  it "renders new account_sub_group form" do
    render
    
    response.should have_tag("form[action=?][method=post]", account_sub_groups_path) do
